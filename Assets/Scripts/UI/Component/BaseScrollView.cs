...
public class BaseScrollView : MonoBehaviour {
    ...
    public void SetData(IList datas)
    {
        ...
        for (int i = 0; i < datas.Count; i++)
        {
            ...
            item.rect = new Rect(offset.x, offset.y, itemTemplate.rect.width, itemTemplate.rect.height) ;
            item.data = datas[i];
            item.isVisable = false;
            itemDatas.Add(item);

            offset += direction == Direction.Horizontal ? new Vector2(itemTemplate.rect.width + spacing,0):new Vector2(0, -(itemTemplate.rect.height + spacing));
        }


        content.sizeDelta = new Vector2(offset.x,-offset.y);
